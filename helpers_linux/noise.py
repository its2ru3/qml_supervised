...
def create_noisy_sampler(cnot_error=0.0373, single_qubit_error=0.001,
                          ...
                          shots=1024, **kwargs):
    """Create a noisy sampler compatible with VQC (BaseSamplerV2).

    Returns a SamplerV2 instance from qiskit_aer.primitives that wraps an
    AerSimulator configured with the paper's noise parameters. This sampler
    can be used directly as the ``sampler`` argument to VQC.
    """
    ...
    sampler = SamplerV2.from_backend(sim)
